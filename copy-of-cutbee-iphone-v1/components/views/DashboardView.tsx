...
import { Chart as ChartJS, CategoryScale, LinearScale, BarElement, Title, Tooltip, Legend, PointElement, LineElement, BarController, LineController, Filler } from 'chart.js';
import { Chart } from 'react-chartjs-2';
import ChartDataLabels from 'chartjs-plugin-datalabels';
import { Hand, ArrowUp, ArrowDown, Minus } from 'lucide-react';

ChartJS.register(CategoryScale, LinearScale, BarElement, Title, Tooltip, Legend, PointElement, LineElement, BarController, LineController, ChartDataLabels, Filler);
...
export default function DashboardView({ data, metrics, lookup, memberInfo, onOpenLostAnalysis, allData, isMobile }: DashboardViewProps) {
  ...
  // Calculate Metrics for Chart (Filtered by Store)
  const chartDisplayedMetrics = useMemo(() => {
      if (chartStoreFilter === 'all') return metrics;
      const storeData = data.filter(o => o.store === chartStoreFilter);
      return calculateMetrics(storeData, lookup);
  }, [chartStoreFilter, metrics, data, lookup]);

  // Chart Data
  const chartData = useMemo(() => {
      // Use metrics derived from chart filter
      const mData = chartDisplayedMetrics;
      ...
      if (currentMetric === 'count') {
          ...
          datasets.push({
              ...
              data: cumulative,
              borderColor: '#64748b',
              // Dynamic Fill: Green if > 0, Red if < 0 relative to origin
              fill: { target: 'origin', above: 'rgba(16, 185, 129, 0.15)', below: 'rgba(244, 63, 94, 0.15)' },
              borderWidth: 2,
              borderDash: [5, 5],
              tension: 0.4,
              pointRadius: 0,
  ...
  }, [chartDisplayedMetrics, currentMetric, chartStoreFilter]);
  ...
  return (
    <section className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8">
        {/* KPI 1 */}
        <div className="clean-card p-6 md:p-8 bg-white border-l-4 border-l-[#244c5a] shadow-sm flex flex-col h-full">
        ...
        <div className="clean-card p-6 md:p-8 bg-white border-l-4 border-l-emerald-500 shadow-sm flex flex-col h-full">
          <div className="flex items-center justify-between mb-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8">
        {/* KPI 3 */}
        <div onClick={() => onOpenLostAnalysis('onetime')} className="clean-card p-6 md:p-8 bg-white border-l-4 border-l-rose-500 shadow-sm flex flex-col h-full clickable-kpi group relative cursor-pointer hover:-translate-y-1 hover:shadow-md transition-all">
        ...
        {/* KPI 4 */}
        <div onClick={() => onOpenLostAnalysis('churn')} className="clean-card p-6 md:p-8 bg-white border-l-4 border-l-amber-500 shadow-sm flex flex-col h-full clickable-kpi group relative cursor-pointer hover:-translate-y-1 hover:shadow-md transition-all">
            ...
            <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
                <div className="relative w-full md:w-auto">
                    <select 
                        ...
                        className="bg-slate-50 border border-slate-100 rounded-lg px-3 py-2 text-xs font-bold text-slate-700 outline-none cursor-pointer w-full md:w-40"
                    >
                ...
                <div className="flex bg-slate-50 p-1 rounded-lg border border-slate-100 w-full md:w-auto overflow-x-auto">
                    <button onClick={() => setCurrentMetric('count')} className={`sub-tab-btn whitespace-nowrap ${currentMetric === 'count' ? 'active' : ''}`}>服務件數</button>
                    <button onClick={() => setCurrentMetric('newAnalysis')} className={`sub-tab-btn ml-1 whitespace-nowrap ${currentMetric === 'newAnalysis' ? 'active' : ''}`}>新客分析</button>
                    <button onClick={() => setCurrentMetric('retentionAnalysis')} className={`sub-tab-btn ml-1 whitespace-nowrap ${currentMetric === 'retentionAnalysis' ? 'active' : ''}`}>留存分析</button>
                    <button onClick={() => setCurrentMetric('trendAnalysis')} className={`sub-tab-btn ml-1 whitespace-nowrap ${currentMetric === 'trendAnalysis' ? 'active' : ''}`}>趨勢分析</button>
                </div>
            </div>
        </div>
        ...
        <div className="h-[300px] md:h-[350px] w-full relative">
            <Chart type={currentMetric === 'trendAnalysis' ? 'bar' : 'line'} data={chartData} options={{
              ...
              plugins: {
                ...
                datalabels: {
                   align: 'top', offset: 4, 
                   // Force dark color for trendAnalysis bars to be visible, otherwise use dataset color
                   color: (ctx) => {
                       if (currentMetric === 'trendAnalysis' && ctx.dataset.type === 'bar') return '#1e293b'; 
                       return ctx.dataset.borderColor as string;
                   },
                   font: { weight: 'bold', size: 10 }, textAlign: 'center',
                   // Hide odd indices on mobile (matches X axis behavior)
                   display: (ctx) => {
                       if (isMobile && ctx.dataIndex % 2 !== 0) return false;
                       return true;
                   },
                   formatter: (value, ctx) => {
              ...
              scales: {
                ...
                x: { 
                    ...
                    ticks: { 
                        font: { size: 10 },
                        // Hide every 2nd label on mobile
