import React, { useState, useMemo } from 'react';
    ...
    import { ArrowLeft, RefreshCw, ArrowDown, ChevronLeft, ChevronRight } from 'lucide-react';
    ...
    export default function BarberDetailView({ metrics, selectedBarber, onBack, lookup, allData, onOpenHairstyleModal }: BarberDetailViewProps) {
      ...
      // Sort barbers by Store first, then Name
      const barbers = useMemo(() => {
        return Object.values(metrics.barberStats)
            .sort((a, b) => {
      ...
      const handlePrevBarber = () => {
          const currentIndex = barbers.indexOf(currentBarberName);
          const newIndex = (currentIndex - 1 + barbers.length) % barbers.length;
          setCurrentBarberName(barbers[newIndex]);
      };

      const handleNextBarber = () => {
          const currentIndex = barbers.indexOf(currentBarberName);
          const newIndex = (currentIndex + 1) % barbers.length;
          setCurrentBarberName(barbers[newIndex]);
      };
      ...
      const radarData = useMemo(() => {
          ...
          const getRadarData = (stats: any) => {
             ...
             // Order: 回訪率, 回訪指定, 總指定, 效率, 戰力
             return [stats.retention, stats.kpiDesSameTotalPct, stats.totalDesignationRate, sEff, sCombat];
          };
          ...
          if (compareStats) {
              ...
              // Order: 回訪率, 回訪指定, 總指定, 效率, 戰力
              data2 = [g.retention, 20, g.totalDes, 50, 50];
              label2 = '全店平均';
              color2 = '#94a3b8';
              dash2 = [5, 5];
              fill2 = false;
      ...
      const trendData = useMemo(() => {
        ...
        return {
            ...
            datasets: [
                { label: '總指定數', data: trendTotalDes, borderColor: '#FFcd00', tension: 0.4, borderWidth: 3, pointRadius: 0, yAxisID: 'y1' },
                { label: '總回訪率', data: trendRet, borderColor: '#101820', tension: 0.4, borderWidth: 2, pointRadius: 0, yAxisID: 'y' },
                { label: '指定回訪率', data: trendDes, borderColor: '#d97706', tension: 0.4, borderWidth: 2, borderDash: [5, 5], pointRadius: 0, yAxisID: 'y' },
      ...
      const kpis = [
          { typeKey: 'iron', label: '回訪後指定 (鐵粉)', pct: barberStats.kpiDesSameTotalPct, desc: '指定回訪人次 / 總服務客數', type: 'good', color: 'text-emerald-600', border: 'border-emerald-500', barColor: 'bg-emerald-500', stats: ironStats },
          { typeKey: 'easy', label: '回訪後不指定 (隨和)', pct: barberStats.kpiRetainedOtherTotalPct, desc: '隨和回訪人次 / 總服務客數', type: 'neutral', color: 'text-amber-500', border: 'border-amber-400', barColor: 'bg-amber-500', stats: easyStats },
          { typeKey: 'lost', label: '沒有回訪 (流失)', pct: barberStats.kpiLostTotalPct, desc: '未回訪人次 / 總服務客數', type: 'bad', color: 'text-rose-500', border: 'border-rose-400', barColor: 'bg-rose-500', stats: lostStats }
      ];
        ...
        <div className="space-y-10 animate-fade">
             <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                 <div className="lg:col-span-1 space-y-6">
                    <div className="clean-card p-10 bg-slate-50 border-none shadow-inner">
                        <div className="flex items-center justify-between mb-4">
                            <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">深度剖析對象</p>
                            <div className="flex items-center space-x-2">
                                <button onClick={handlePrevBarber} className="p-1 rounded-full hover:bg-slate-200 transition text-slate-500">
                                    <ChevronLeft size={16} />
                                </button>
                                <button onClick={handleNextBarber} className="p-1 rounded-full hover:bg-slate-200 transition text-slate-500">
                 ...
                 <div className="lg:col-span-2 space-y-6">
                    {/* Reduce padding on mobile (p-4 -> p-2 or less for chart container) */}
                    <div className="clean-card p-2 md:p-4 bg-white h-[450px] md:h-[500px] flex flex-col">
                        <div className="flex justify-between items-start mb-4 px-2 md:px-4 pt-2">
                            <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest">綜合戰力雷達 (PK 模式)</h4>
                            <div className="flex items-center gap-2 bg-slate-50 p-1 rounded-lg border border-slate-100">
                                <span className="text-[9px] font-bold text-slate-400 uppercase px-2 hidden md:inline">Compare:</span>
                                <select 
                                    value={compareName}
                                    onChange={(e) => setCompareName(e.target.value)}
                                    className="bg-transparent text-xs font-bold text-slate-700 outline-none w-24 md:w-32 cursor-pointer"
                                >
                        ...
                        <div className="flex-grow flex items-center justify-center relative w-full h-full">
                            <Radar data={radarData} options={{ responsive: true, maintainAspectRatio: false, layout: { padding: 0 }, scales: { r: { min: 0, max: 100, ticks: { display: false, stepSize: 20 }, pointLabels: { font: { size: 10, weight: 'bold' } } } }, plugins: { legend: { position: 'bottom', labels: { boxWidth: 8, font: { size: 10 } } }, datalabels: { display: false } } }} />
                        </div>
                    </div>
                 </div>
             </div>
                        ...
                        >
                            <div className="flex-1 w-full">
                                ...
                                <div className="flex items-baseline gap-0">
                                    <span className={`text-3xl font-bold ${k.color} inline-block w-[110px]`}>{k.pct.toFixed(1)}%</span>
                                    <span className="text-[10px] font-medium text-slate-400 hidden lg:inline-block">{k.desc}</span>
                                </div>
                            </div>
                            
                            {/* Stats Detail with Progress Bar - Visible on all devices now */}
                            <div className="w-full xl:w-auto xl:border-l xl:border-slate-100 xl:pl-4 xl:ml-2">
                                <div className="mt-0 pl-0 xl:pl-6 xl:ml-4 flex flex-col justify-center gap-2 w-full xl:w-40 border-l-0 xl:border-l border-slate-100">
                                    {/* TOP Stats */}
                                    <div className="w-full">
                                        ...
                                        <div className="w-full h-1.5 rounded-full bg-slate-100">
                                            <div className={`h-1.5 rounded-full ${k.barColor}`} style={{ width: `${k.stats.topMax.pct}%` }}></div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="w-full">
                                        ...
                                        <div className="w-full h-1.5 rounded-full bg-slate-100">
                                            <div className={`h-1.5 rounded-full ${k.barColor}`} style={{ width: `${k.stats.sideMax.pct}%` }}></div>
                                        </div>
                                    </div>
                                </div>
                            </div>
             ...
             <div className="clean-card p-8 bg-white h-[400px]">
                ...
                <div className="h-[320px] w-full">
                    <Line data={trendData} options={{ responsive: true, maintainAspectRatio: false, scales: { y: { beginAtZero: true, max: 100 }, y1: { type: 'linear', display: false, position: 'right', beginAtZero: true }, x: { grid: { display: false } } }, plugins: { legend: { position: 'top', align: 'end', labels: { boxWidth: 10, font: { size: 10 } } }, datalabels: { display: false } } }} />
                </div>
            </div>
