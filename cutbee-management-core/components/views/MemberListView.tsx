...
export default function MemberListView({ allData, memberData }: MemberListViewProps) {
  ...
  const ageChartData = useMemo(() => {
    ...
    // Use filtered members to reflect current view (or use memberStats for global)
    // Here using memberStats (global) but could act on filtered.
    // Generally demographics are best shown for the filtered set or global active set.
    // Let's use 'filteredMembers' so the chart updates when you search or filter status!
            ...
            {[
                ...
                return (
                    <div 
                        ...
                        className={`clean-card p-8 text-center cursor-pointer transition border-b-4 ${borderClass} ${bgClass} transform duration-300 ${isDimmed ? 'opacity-40 grayscale' : ''} ${isActive || !filterStatus ? 'scale-100' : ''} ${isActive ? 'scale-105 shadow-lg' : ''}`}
                    >
                        <p className="text-[9px] font-bold text-slate-400 uppercase mb-2">{card.label}</p>
                        <div className="text-4xl font-bold text-slate-900 mb-1">{card.count.toLocaleString()}</div>
        ...
        {ageChartData && (
            <div className="clean-card p-8 bg-white h-[400px]">
                <div className="flex flex-col md:flex-row justify-between items-center mb-4">
                    <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2 md:mb-0">會員年齡層分布 (依最後消費店家區分)</h4>
        ...
        <div className="clean-card p-10">
            <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-6">
                ...
                <div className="flex items-center gap-4">
                    <div className="text-[9px] font-bold text-slate-400 bg-slate-50 px-4 py-2 rounded-full border border-slate-100 uppercase tracking-widest">Matched: {filteredMembers.length}</div>
                </div>
            </div>
                ...
                <table className="clean-table">
                    ...
                    <tbody>
                        {currentItems.map(m => {
                            ...
                            return (
                                <tr key={m.id} className="cursor-pointer hover:bg-indigo-50/50 transition">
                                    <td className="font-bold text-slate-800">{m.name}</td>
                                    <td className="font-mono text-slate-400 text-xs">{m.id}</td>
                                    <td className="text-center font-bold">{m.count}</td>
