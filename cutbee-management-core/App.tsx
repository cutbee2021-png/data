        ...
        <header className="bg-white border-b border-slate-100 h-20 sticky top-0 z-50">
            <div className="container mx-auto px-8 h-full flex items-center justify-between">
                <div className="flex items-center space-x-4">
                    <div className="w-9 h-9 bg-[#244c5a] rounded-lg flex items-center justify-center text-white shadow-lg">
                        <BarChart size={18} />
                    </div>
                    <div>
                        <h1 className="text-lg font-bold tracking-tight text-slate-900 leading-none">CUTBEE</h1>
                        <span className="text-[9px] font-bold text-[#244c5a] uppercase tracking-widest">Management Core</span>
                    </div>
                </div>
                ...
                <div className="flex items-center space-x-6">
                    {allData.length > 0 && (
                        <div className="relative">
                            <select 
                                ...
                                className="bg-white border border-slate-200 rounded-lg pl-4 pr-10 py-2 text-xs font-bold text-slate-700 outline-none cursor-pointer hover:border-[#244c5a] transition shadow-sm appearance-none focus:ring-2 focus:ring-[#244c5a]/20"
                            >
                                <option value="all">全時段 (All Years)</option>
                    ...
                    <div className="relative">
                        <label className={`bg-slate-200 text-slate-400 text-[10px] font-bold px-4 py-2 rounded-lg cursor-pointer hover:bg-slate-300 transition shadow-sm flex items-center ${Object.keys(memberData).length > 0 ? '!bg-[#244c5a] !text-white' : ''}`}>
                    ...
                    <div className="relative">
                        <label className={`bg-slate-200 text-slate-400 text-[10px] font-bold px-4 py-2 rounded-lg cursor-pointer hover:bg-slate-300 transition shadow-sm flex items-center ${allData.length > 0 ? '!bg-slate-900 !text-white' : ''}`}>
        ...
        <main className="container mx-auto px-8 py-10">
            {allData.length === 0 ? (
                ...
                <div className="animate-fade">
                    ...
                    {activeTab === 'member-list' && <MemberListView allData={enrichedAllData} memberData={memberData} />}
                </div>
            )}
        </main>
